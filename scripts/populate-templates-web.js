/**
 * Web-based Template Population Script
 * 
 * This creates a simple HTML file that can be opened in a browser
 * to populate the template database using the existing Firebase config.
 */
...
<body>
    <h1>🏠 Family Compass - Template Database Population</h1>
    ...
    <script type="module">
        ...
        window.testConnection = async function() {
            ...
            try {
                ...
                log(\`✅ Connection successful! Found \${snapshot.size} existing templates.\`, 'success');
            } catch (error) {
        ...
        window.populateTemplates = async function() {
            ...
            try {
                log('🏠 Starting template database population...');
                ...
                for (const template of templates) {
                    try {
                        ...
                        if (existingSnap.size > 0) {
                            log(\`⏭️ Skipping "\${template.name}" - already exists\`, 'warning');
                            skippedCount++;
                            continue;
                        }
                        ...
                        log(\`✅ Added "\${template.name}" (\${template.category}) - \${docRef.id}\`, 'success');
                        addedCount++;

                    } catch (error) {
                        log(\`❌ Error adding template "\${template.name}": \${error.message}\`, 'error');
                    }
                }

                log(\`\\n📊 Population Summary:\`, 'info');
                log(\`✅ Templates added: \${addedCount}\`, 'success');
                log(\`⏭️ Templates skipped: \${skippedCount}\`, 'warning');
                
                if (addedCount > 0) {
                    log(\`\\n🎉 Template database population completed successfully!\`, 'success');
                    log(\`Templates are now available in the Family Compass app.\`, 'info');
                } else {
                    log(\`\\n✨ No new templates needed - database is already populated!\`, 'info');
                }

            } catch (error) {
                log(\`💥 Fatal error during template population: \${error.message}\`, 'error');
            } finally {
                populateBtn.disabled = false;
            }
        };

        log('✅ Firebase initialized successfully. Ready to populate templates!', 'success');
    </script>
</body>
</html>
`;
...
// Write the HTML file
const outputPath = path.join(__dirname, '..', 'public', 'populate-templates.html');
fs.writeFileSync(outputPath, htmlContent);

console.log('✅ Created web-based template population script at: public/populate-templates.html');
console.log('📱 To use:');
console.log('1. Run: npm start');
console.log('2. Open: http://localhost:8081/populate-templates.html');
console.log('3. Click "Populate Templates" to add templates to the database');